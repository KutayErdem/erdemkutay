...
public class FavoritesPlayerActivity extends YouTubeBaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_favorites_player);
        ...
        favs = findViewById(R.id.remove_favs);
        favs.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sharedPref = getSharedPreferences("UserFavorites", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = sharedPref.edit();
                editor.remove(username + password + videoID);
                editor.commit();
                Toast.makeText(getApplicationContext(),"Video is removed\n  from Favorites", Toast.LENGTH_LONG).show();
            }
        });
    ...
    public void onBackPressed() {
       if (activity_name.equals("favorites")) {
           startActivity(new Intent(FavoritesPlayerActivity.this, FavoritesActivity.class)
                   .putExtra("user_name", username).putExtra("pass_word", password)
                   .putExtra("namesurname_", namesurname).putExtra("email_", email));
       }
       else if (activity_name.equals("login_main")) {
            startActivity(new Intent(FavoritesPlayerActivity.this, LoginMainActivity.class)
                    .putExtra("_username", username).putExtra("_password", password)
                    .putExtra("_name_surname", namesurname).putExtra("_email", email));
        }
       else if (activity_name.equals("login_math")) {
            startActivity(new Intent(FavoritesPlayerActivity.this, LoginMathActivity.class)
                    .putExtra("_username", username).putExtra("_password", password)
                    .putExtra("_name_surname", namesurname).putExtra("_email", email));
        }
        else if (activity_name.equals("login_engineering")) {
            startActivity(new Intent(FavoritesPlayerActivity.this, LoginEngineeringActivity.class)
                    .putExtra("_username", username).putExtra("_password", password)
                    .putExtra("_name_surname", namesurname).putExtra("_email", email));
        }
       else if (activity_name.equals("login_science")) {
           startActivity(new Intent(FavoritesPlayerActivity.this, LoginScienceActivity.class)
                   .putExtra("_username", username).putExtra("_password", password)
                   .putExtra("_name_surname", namesurname).putExtra("_email", email));
       }
    }
